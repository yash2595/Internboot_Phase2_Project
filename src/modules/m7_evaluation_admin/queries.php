...
function get_batches(mysqli $conn): array
{
    ...
    $eligible = q_all($conn, "SELECT c.id candidate_id, c.full_name, u.email, e.id enrollment_id,
        ...
        b.batch_number, s.exam_date, es.start_time, es.end_time,
        (SELECT r.level_assigned FROM results r JOIN attempts ax ON ax.id=r.attempt_id WHERE ax.candidate_id=c.id ORDER BY r.created_at DESC LIMIT 1) level_assigned,
        COALESCE((SELECT p.status FROM payments p WHERE p.candidate_id=c.id ORDER BY p.id DESC LIMIT 1),'pending') payment_status
        FROM enrollments e
...
function create_batch(mysqli $conn, string $batchNumber, int $assessmentId, string $examDate, int $capacity, string $startTime = '10:00:00', string $endTime = '11:00:00'): array
{
    ...
    // Normalize start and end times to H:i:s
    ...
    try{
        ...
        // Single Slot per Batch (1 Batch = 1 Unified Slot, no split slots)
        $stmt=$conn->prepare('INSERT INTO exam_slots(exam_schedule_id,start_time,end_time,capacity,seats_remaining) VALUES(?,?,?,?,?)');
        $stmt->bind_param('issii',$scheduleId,$startTime,$endTime,$capacity,$capacity); $stmt->execute(); $slotId=$stmt->insert_id; $stmt->close();
        ...
        return ['batch_id'=>$batchId,'batch_number'=>$batchNumber,'assessment_id'=>$assessmentId,'assessment_title'=>$assessment['title'],'schedule_id'=>$scheduleId,'exam_date'=>$examDate,'capacity'=>$capacity,'slots'=>[
            ['slot_id'=>$slotId,'start_time'=>$startTime,'end_time'=>$endTime,'capacity'=>$capacity,'seats_remaining'=>$capacity]
        ]];
    }catch(Throwable $e){$conn->rollback();throw $e;}
}

/**
 * Reassigns one or more candidates from their current batch to a target batch,
 * automatically updating attempts and synchronizing capacity of both source and target slots.
 */
function reassign_candidates_batch(mysqli $conn, array $enrollmentIds, int $targetBatchId): array
{
    if (empty($enrollmentIds)) {
        throw new InvalidArgumentException('No candidates selected for reassignment.');
    }

    $targetBatch = q_one($conn, 'SELECT id, batch_number, assessment_id FROM batches WHERE id=?', 'i', [$targetBatchId]);
    if (!$targetBatch) {
        throw new InvalidArgumentException('Target batch does not exist.');
    }

    // Get target batch unified slot
    $targetSlot = q_one($conn, 'SELECT es.id slot_id, es.capacity, es.seats_remaining, s.exam_date, es.start_time, es.end_time
        FROM exam_slots es
        JOIN exam_schedules s ON s.id=es.exam_schedule_id
        WHERE s.batch_id=? AND s.status <> "cancelled"
        ORDER BY es.id ASC LIMIT 1', 'i', [$targetBatchId]);

    if (!$targetSlot) {
        throw new InvalidArgumentException('Target batch does not have an active exam slot.');
    }

    $targetSlotId = (int)$targetSlot['slot_id'];
    $sourceSlotIdsToUpdate = [];
    $movedCount = 0;

    $conn->begin_transaction();
    try {
        foreach ($enrollmentIds as $eid) {
            $enrollmentId = (int)$eid;
            if ($enrollmentId <= 0) continue;

            $enr = q_one($conn, 'SELECT id, candidate_id, batch_id FROM enrollments WHERE id=? OR candidate_id=?', 'ii', [$enrollmentId, $enrollmentId]);
            if (!$enr) continue;

            $oldBatchId = $enr['batch_id'] ? (int)$enr['batch_id'] : null;
            $candidateId = (int)$enr['candidate_id'];
            $realEnrollmentId = (int)$enr['id'];
            if ($oldBatchId === $targetBatchId) continue;

            // Update enrollment to target batch
            $uStmt = $conn->prepare('UPDATE enrollments SET batch_id=?, updated_at=NOW() WHERE id=?');
            $uStmt->bind_param('ii', $targetBatchId, $realEnrollmentId);
            $uStmt->execute();
            $uStmt->close();

            // Check active attempt
            $att = q_one($conn, 'SELECT id, exam_slot_id FROM attempts WHERE candidate_id=? AND status="in_progress" ORDER BY id DESC LIMIT 1', 'i', [$candidateId]);
            if ($att) {
                $oldSlotId = (int)$att['exam_slot_id'];
                if ($oldSlotId > 0 && $oldSlotId !== $targetSlotId) {
                    $sourceSlotIdsToUpdate[$oldSlotId] = true;
                }
                $attStmt = $conn->prepare('UPDATE attempts SET exam_slot_id=?, updated_at=NOW() WHERE id=?');
                $attStmt->bind_param('ii', $targetSlotId, $att['id']);
                $attStmt->execute();
                $attStmt->close();
            } else {
                ...
                $insAtt->bind_param('iii', $candidateId, $targetBatch['assessment_id'], $targetSlotId);
                $insAtt->execute();
                $insAtt->close();
            }
        ...
        // Synchronize remaining seats in source slots
        foreach (array_keys($sourceSlotIdsToUpdate) as $srcSlotId) {
            $totalAttempts = (int)(q_value($conn, 'SELECT COUNT(*) FROM attempts WHERE exam_slot_id=? AND status="in_progress"', 'i', [$srcSlotId]) ?? 0);
            $conn->query("UPDATE exam_slots SET seats_remaining = GREATEST(0, capacity - {$totalAttempts}), updated_at=NOW() WHERE id = {$srcSlotId}");
        }

        // Dynamically synchronize target slot capacity so all allocated students are accommodated
        $totalEnrolledInTarget = (int)(q_value($conn, 'SELECT COUNT(*) FROM enrollments WHERE batch_id=?', 'i', [$targetBatchId]) ?? 0);
        $newTargetCap = max((int)$targetSlot['capacity'], $totalEnrolledInTarget);
        $totalAttemptsInTarget = (int)(q_value($conn, 'SELECT COUNT(*) FROM attempts WHERE exam_slot_id=? AND status="in_progress"', 'i', [$targetSlotId]) ?? 0);
        $newSeatsRem = max(0, $newTargetCap - $totalAttemptsInTarget);

        $syncStmt = $conn->prepare('UPDATE exam_slots SET capacity=?, seats_remaining=?, updated_at=NOW() WHERE id=?');
        $syncStmt->bind_param('iii', $newTargetCap, $newSeatsRem, $targetSlotId);
        $syncStmt->execute();
        $syncStmt->close();
        ...
        return [
            ...
            'updated_capacity' => $newTargetCap,
            'seats_remaining' => $newSeatsRem
        ];
    } catch (Throwable $e) {
        $conn->rollback();
        throw $e;
