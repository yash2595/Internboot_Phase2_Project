  ...
  async function loadBatches() {
    ...
    if (slotBody)
      ...
      <tr class="hover:bg-slate-50">
        ...
        <td class="px-6 py-4 text-slate-500">${formatDate(s.exam_date)}</td>
        <td class="px-6 py-4 text-slate-500">${escapeHtml(s.start_time?.slice(0, 5) || "")} - ${escapeHtml(s.end_time?.slice(0, 5) || "")}</td>
        <td class="px-6 py-4">${s.capacity}</td>
        <td class="px-6 py-4">${s.allocated}</td>
        <td class="px-6 py-4">${s.seats_remaining}</td>
      ...
      allocBody.innerHTML = unallocated.length
        ...
        <td class="px-6 py-4">
          <select class="allocation-slot rounded-lg border border-slate-200 px-2 py-2 text-xs" data-enrollment="${c.enrollment_id}">
            ...
            ${matchingSlots.map((s) => `<option value="${s.slot_id}">${escapeHtml(s.batch_number)} · ${formatDate(s.exam_date)} · ${escapeHtml(s.start_time.slice(0, 5))} (${s.seats_remaining} left)</option>`).join("")}
          </select>
        </td>
    ...
    // Populate Batch Filter and Target Batch Dropdowns for Manual Reassignment
    const reassignFilter = $("#reassignBatchFilter");
    const targetBatchSelect = $("#reassignTargetBatchSelect");
    if (reassignFilter) {
      const currentVal = reassignFilter.value;
      reassignFilter.innerHTML = '<option value="">All Batches</option>' +
        batches.map((b) => `<option value="${b.id}">${escapeHtml(b.batch_number)} (${formatDate(b.exam_date)})</option>`).join("");
      reassignFilter.value = currentVal;
    }
    if (targetBatchSelect) {
      targetBatchSelect.innerHTML = '<option value="">Select Target Batch...</option>' +
        batches.map((b) => `<option value="${b.id}">${escapeHtml(b.batch_number)} · ${formatDate(b.exam_date)}</option>`).join("");
    }

    // Render Candidates Assigned to Batches for Override / Reassignment
    const reassignBody = $("#reassignTableBody");
    if (reassignBody) {
      const assigned = eligible.filter((c) => c.batch_id);
      const filterBatchId = reassignFilter ? reassignFilter.value : "";
      const filteredAssigned = filterBatchId
        ? assigned.filter((c) => String(c.batch_id) === String(filterBatchId))
        : assigned;

      reassignBody.innerHTML = filteredAssigned.length
        ? filteredAssigned
            .map(
              (c) => `
          <tr class="hover:bg-slate-50">
            <td class="px-4 py-3"><input type="checkbox" class="reassign-checkbox rounded" data-enrollment="${c.enrollment_id}"></td>
            <td class="px-6 py-3 font-medium">${escapeHtml(c.full_name)}</td>
            <td class="px-6 py-3 text-slate-500">${escapeHtml(c.email)}</td>
            <td class="px-6 py-3"><span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold bg-blue-50 text-blue-700">${escapeHtml(c.batch_number || "Batch #" + c.batch_id)}</span></td>
            <td class="px-6 py-3 text-slate-500 text-xs">${formatDate(c.exam_date)} ${c.start_time ? `(${escapeHtml(c.start_time.slice(0, 5))} - ${escapeHtml(c.end_time?.slice(0, 5) || "")})` : ""}</td>
            <td class="px-6 py-3">
              <select class="quick-reassign-select rounded border border-slate-200 px-2 py-1 text-xs" data-enrollment="${c.enrollment_id}">
                <option value="">Move to...</option>
                ${batches.filter((b) => Number(b.id) !== Number(c.batch_id)).map((b) => `<option value="${b.id}">${escapeHtml(b.batch_number)}</option>`).join("")}
              </select>
            </td>
          </tr>`,
            )
            .join("")
        : '<tr><td colspan="6" class="px-6 py-8 text-center text-sm text-slate-500">No assigned candidates found for this selection.</td></tr>';

      $$(".quick-reassign-select").forEach((sel) => {
        sel.onchange = async () => {
          const targetId = Number(sel.value);
          if (!targetId) return;
          const enrollmentId = Number(sel.dataset.enrollment);
          try {
            const res = await api("reassign-batch", {
              method: "POST",
              body: { enrollment_ids: [enrollmentId], target_batch_id: targetId },
            });
            notify(res.message || "Candidate moved successfully.");
            await loadBatches();
          } catch (e) {
            notify(e.message, true);
          }
    ...
    if (reassignFilter && !reassignFilter.dataset.bound) {
      reassignFilter.dataset.bound = "1";
      reassignFilter.onchange = () => loadBatches();
    }

    const selectAll = $("#selectAllReassign");
    if (selectAll && !selectAll.dataset.bound) {
      selectAll.dataset.bound = "1";
      selectAll.onchange = () => {
        $$(".reassign-checkbox").forEach((cb) => {
          cb.checked = selectAll.checked;
        });
      };
    }

    const bulkBtn = $("#bulkReassignBtn");
    if (bulkBtn && !bulkBtn.dataset.bound) {
      bulkBtn.dataset.bound = "1";
      bulkBtn.onclick = async () => {
        const targetSelect = $("#reassignTargetBatchSelect");
        const targetId = Number(targetSelect?.value || 0);
        if (!targetId) {
          notify("Please select a target batch first.", true);
          return;
        }
        const selectedIds = $$(".reassign-checkbox:checked").map((cb) =>
          Number(cb.dataset.enrollment),
        );
        if (selectedIds.length === 0) {
          notify("Please select at least one candidate to move.", true);
          return;
        }
        try {
          const res = await api("reassign-batch", {
            method: "POST",
            body: { enrollment_ids: selectedIds, target_batch_id: targetId },
          });
          notify(res.message || `Moved ${selectedIds.length} candidate(s) successfully.`);
          await loadBatches();
        } catch (e) {
          notify(e.message, true);
        }
    ...
    // Flatpickr configuration: only Saturdays and Sundays enabled; Monday-Friday greyed out & unclickable
    ...
    if (batchDate && !batchDate.dataset.bound) {
      ...
      if (typeof flatpickr !== "undefined") {
        flatpickr(batchDate, {
          ...
          enable: [
            function (date) {
              return date.getDay() === 0 || date.getDay() === 6; // Sunday or Saturday only
            },
          ],
        });
      }
    ...
    if (form && !form.dataset.bound) {
      ...
      form.addEventListener("submit", async (e) => {
        ...
        const name = $("#batchName")?.value.trim(),
          date = $("#batchDate")?.value.trim(),
          startTime = $("#batchStartTime")?.value.trim() || "10:00",
          endTime = $("#batchEndTime")?.value.trim() || "11:00",
          capacity = Number($("#batchCapacity")?.value || 0);

        if (!name) {
          notify("Please enter a batch name.", true);
          return;
        }
        if (!date) {
          notify("Please select an upcoming Saturday or Sunday.", true);
          return;
        }
        ...
        if (!isWeekend) {
          notify("Assessment can be scheduled only on Saturday or Sunday.", true);
          return;
        }
        if (capacity < 100) {
          notify("Batch capacity must be at least 100.", true);
          return;
        }
        try {
          await api("batch", {
          ...
          notify("Batch created successfully with 1 unified slot.");
          form.reset();
          $("#batchCapacity").value = 100;
          $("#batchStartTime").value = "10:00";
          $("#batchEndTime").value = "11:00";
          await loadBatches();
        } catch (err) {
          notify(err.message, true);
        }
